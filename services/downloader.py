...
def get_ydl_options(job_dir: Path | None = None, progress_data: dict | None = None, mode: str = "video", resolution: str = "720"):
    ...
    if mode == "audio":
        ...
        # 🌟 الاستراتيجية الذكية للدقة الحقيقية متوافقة مع جميع المنصات (H.264 القياسي):
        if resolution == "best":
            opts["format"] = (
                ...
                f"best[filesize<?{max_fs}][ext=mp4]/"
                f"best"
            )
        else:
            opts["format"] = (
                ...
                f"best[height<={resolution}][filesize<?{max_fs}][ext=mp4]/"
                f"best"
            )
            
        opts["merge_output_format"] = "mp4"
        # ضمان معالجة الصوت بترميز AAC عالي الجودة متوافق مع كافة الهواتف مع تثبيت تزامن الفيديو
