...
def convert_to_mp3_local(input_file: Path, output_file: Path, local_thumb: Path = None) -> bool:
    try:
        ...
        # التأكد من وجود الصورة المصغرة وأن حجمها أكبر من 0
        if local_thumb and local_thumb.exists() and local_thumb.stat().st_size > 0:
            # إضافة علامة الاستفهام ? للمسارات لتجنب توقف العملية إذا كان المسار مفقوداً
        ...
        try:
            # المحاولة الأولى: التحويل مع دمج الصورة المصغرة
            ...
            # نظام الإنقاذ (Fallback): التحويل الصافي بدون الصورة المصغرة
            fallback_cmd = [
        ...
        logger.error(f"فشل التحويل المحلي لـ MP3 بالكامل: {e}")
        return False
