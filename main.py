...
# =========================================================
# Telegram Audio/Video Bot - PlayZone Style
# =========================================================
...
APP_NAME = os.getenv("APP_NAME", "بوت تنزيل تحميل اغاني من يوتيوب !")
...
BLOCKED_PLATFORM_KEYWORDS = [
    ...
    "fb.watch", "x.com", "twitter.com", "soundcloud.com", "spotify.com",
...
DEFAULT_ALLOWED_DOMAINS = [
    ...
    "freemusicarchive.org", "www.freemusicarchive.org",
...
def register_user(message: Message):
    ...
    with db_connect() as con:
        ...
        if exists:
            ...
            cur.execute("""
                INSERT INTO users 
                (user_id, username, full_name, first_seen, last_seen, total_requests)
                VALUES (?, ?, ?, ?, ?, 1)
...
def set_user_block(user_id: int, blocked: bool):
    with db_connect() as con:
        ...
        cur.execute("""
            INSERT INTO users 
            (user_id, username, full_name, first_seen, last_seen, is_blocked)
            VALUES (?, '', '', ?, ?, ?)
...
def set_follow_confirmed(user_id: int):
    with db_connect() as con:
        cur = con.cursor()
        cur.execute(
            "UPDATE users SET follow_confirmed = 1, follow_confirmed_at = ? WHERE user_id = ?",
            (now_text(), user_id)
        )
        con.commit()
...
def add_download(user_id: int, kind: str, source: str, title: str = "") -> int:
    with db_connect() as con:
        ...
        cur.execute("""
            ...
            (user_id, kind, source, title, status, created_at)
            VALUES (?, ?, ?, ?, 'processing', ?)
...
def finish_download(download_id: int, user_id: int, status: str, size_mb: float = 0, error: str = ""):
    with db_connect() as con:
        ...
        cur.execute("""
            UPDATE downloads 
            SET status = ?, size_mb = ?, finished_at = ?, error = ? 
            WHERE id = ?
...
def user_downloads_today(user_id: int) -> int:
    with db_connect() as con:
        cur = con.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM downloads WHERE user_id = ? AND substr(created_at, 1, 10) = ?",
            (user_id, today_text())
        )
...
# =========================================================
# UI - نفس شكل الصور تقريباً
# =========================================================

def follow_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 اشترك في قناة البوت", url=TELEGRAM_URL)],
        [InlineKeyboardButton(text="📸 تابع Instagram", url=INSTAGRAM_URL)],
        [InlineKeyboardButton(text="✅ اشتركت", callback_data="check_follow")],
    ])


def main_menu(user_id: int):
    rows = [
        [InlineKeyboardButton(text="القائمة", callback_data="download_home")],
        [InlineKeyboardButton(text="📣 استعراض خيارات الإعلان", callback_data="ads")],
    ]
...
def download_home_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📎 أرسل ملف صوتي أو فيديو", callback_data="help_file")],
        [InlineKeyboardButton(text="🔗 أرسل رابط مباشر", callback_data="help_links")],
        [InlineKeyboardButton(text="🔎 بحث باسم أغنية", callback_data="search_name")],
...
def result_buttons():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="مقطع صوتي", callback_data="send_as_audio_clip"),
            InlineKeyboardButton(text="ملف صوتي", callback_data="send_as_audio_file"),
        ],
        [InlineKeyboardButton(text="مقطع فيديو", callback_data="send_as_video")],
        [InlineKeyboardButton(text="مشاركة ✉️", callback_data="share_info")],
        [InlineKeyboardButton(text="• رجوع •", callback_data="download_home")],
    ])
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="📅 شهرياً", callback_data="admin_monthly"),
        ],
        [
            InlineKeyboardButton(text="🌐 المواقع", callback_data="admin_domains"),
            InlineKeyboardButton(text="⚙️ الإعدادات", callback_data="admin_settings"),
        ],
        [
            InlineKeyboardButton(text="🔐 شرط المتابعة", callback_data="admin_follow_toggle"),
            InlineKeyboardButton(text="🟢/🔴 الصيانة", callback_data="admin_maintenance"),
        ],
...
def clean_domain(domain: str) -> str:
    ...
    domain = domain.replace(":443", "").replace(":80", "")
    return domain
...
def admin_stats_text() -> str:
    ...
    return (
        ...
        f"🔐 أكدوا المتابعة: {followers_confirmed}\n"
        f"🚫 المحظورين: {blocked_users}\n"
        f"⬇️ إجمالي الطلبات: {total_requests}\n"
        f"📅 طلبات هذا الشهر: {requests_month}\n"
        f"✅ ناجحة: {success}\n"
        f"❌ فاشلة: {failed}\n"
        f"📈 نسبة النجاح: {rate}%\n"
        f"💾 الحجم المرسل: {round(total_mb, 2)} MB"
    )
...
def monthly_stats_text() -> str:
    ...
    for month, requests, active_users, success in rows:
        text += (
            f"🗓 {month}\n"
            f"⬇️ {monthly_bar(requests, max_requests)} {requests} طلب\n"
            f"👥 {monthly_bar(active_users, max_users)} {active_users} مستخدم\n"
            f"✅ ناجح: {success or 0}\n\n"
        )
...
def user_stats_text(user_id: int) -> str:
    with db_connect() as con:
        ...
        cur.execute("""
            SELECT total_requests, total_success, total_failed, first_seen, last_seen, follow_confirmed
            FROM users
            WHERE user_id = ?
        """, (user_id,))
        ...
        cur.execute("""
            SELECT COUNT(*), IFNULL(SUM(size_mb), 0)
            FROM downloads
    ...
    return (
        ...
        f"📨 الطلبات: {total_requests}\n"
        f"✅ الناجحة: {total_success}\n"
        f"❌ الفاشلة: {total_failed}\n"
        f"💾 الحجم المرسل: {round(total_mb, 2)} MB\n"
...
def settings_text() -> str:
    return (
        ...
        f"📅 الحد اليومي لكل مستخدم: {get_daily_limit()} طلب\n"
        f"🟢 الصيانة: {get_setting('maintenance', 'off')}\n"
        f"🔐 شرط المتابعة: {get_setting('force_follow', 'on')}\n"
        f"🔗 قبول الروابط المباشرة: {get_setting('allow_direct_links', 'on')}\n\n"
        "أوامر الإدارة:\n"
        "/setmax 45\n"
        "/setlimit 20\n"
...
async def start(message: Message):
    ...
    await message.answer(
        "👀 هل ترغب أن يظهر إعلانك في هذا المكان؟\n\n"
        "📣 هذا الإعلان يُعرض لمستخدمين البوت بشكل منظم.\n\n"
        "✨ احجز إعلانك محدد الوقت الذي يناسبك.",
        reply_markup=main_menu(message.from_user.id)
    )

    await message.answer(
        "• مرحبا بك في قسم تحميل من البوت\n\n"
        "- يمكنك التحميل بعدة طرق:\n\n"
        "1. ارسال ملف صوتي أو فيديو للبوت بشكل مباشر\n"
        "2. ارسال رابط مباشر لملف صوتي أو فيديو\n"
        "3. ارسال كلمة للبحث اليدوي عن النتيجة\n\n"
        "- اختر الطريقة التي تناسبك.",
        reply_markup=download_home_menu()
    )
...
async def help_cmd(message: Message):
    ...
    await message.answer(
        "❓ المساعدة\n\n"
        "ارسل ملف صوتي أو فيديو وسيتم تحويله إلى MP3.\n\n"
        "أو ارسل رابط مباشر مسموح.\n\n"
        "/start - القائمة\n"
        "/stats - إحصائياتك",
        reply_markup=download_home_menu()
    )
...
@dp.callback_query(F.data == "download_home")
async def cb_download_home(call: CallbackQuery):
    if not await require_follow_callback(call):
        return
...
async def cb_ads(call: CallbackQuery):
    await call.message.edit_text(
        "📣 استعراض خيارات الإعلان\n\n"
        "يمكن عرض إعلانك داخل البوت للمستخدمين.\n\n"
        "للحجز راسل الإدارة.",
...
async def cb_help_file(call: CallbackQuery):
    ...
    await call.message.edit_text(
        "📎 ارسل ملف صوتي أو فيديو الآن.\n\n"
        "سأقوم بتحويله إلى MP3 ثم أرسله لك.\n\n"
        "الصيغ المدعومة:\n"
        "MP4 / MOV / WEBM / MP3 / M4A / WAV / OGG / FLAC",
        reply_markup=back_menu()
    )
...
async def cb_help_links(call: CallbackQuery):
    ...
    await call.message.edit_text(
        "🔗 ارسل رابط مباشر لملف صوتي أو فيديو.\n\n"
        "مثال:\n"
        "https://example.com/audio.mp3\n\n"
        "⚠️ روابط المنصات المحمية لا تعمل هنا.\n"
        "الحل: ارسل الملف نفسه للبوت.",
        reply_markup=back_menu()
    )
...
async def cb_search_name(call: CallbackQuery):
    if not await require_follow_callback(call):
        return

    await call.message.edit_text(
        "🔎 ارسل اسم الأغنية أو الفنان.\n\n"
        "سأعرض لك رسالة منظمة، وبعدها يمكنك إرسال الملف أو رابط مباشر.",
        reply_markup=back_menu()
    )
...
async def cb_share_info(call: CallbackQuery):
    await call.answer("يمكنك مشاركة الملف من زر المشاركة في تيليجرام.", show_alert=True)


@dp.callback_query(F.data.in_({"send_as_audio_clip", "send_as_audio_file", "send_as_video"}))
async def cb_fake_result_buttons(call: CallbackQuery):
    await call.answer("أرسل ملفاً أو رابطاً أولاً حتى أجهزه لك.", show_alert=True)
...
async def cb_admin_maintenance(call: CallbackQuery):
    ...
    await call.message.edit_text(
        f"✅ تم تغيير وضع الصيانة إلى: {new_value}",
    ...
    try:
        ...
        if incoming_mb > max_file_mb:
            await status.edit_text(
                f"❌ الملف كبير جداً.\n\n"
                f"📦 حجم الملف: {round(incoming_mb, 2)}MB\n"
        ...
        if output_mb > max_file_mb:
            await status.edit_text(
                f"❌ الملف الناتج كبير جداً.\n\n"
                f"📦 الحجم: {output_mb}MB\n"
                f"🔒 الحد الحالي: {max_file_mb}MB"
            )
        ...
        title = safe_name(Path(original_name).stem)

        await send_final_audio(
            message,
            output_path,
            title,
            f"🎬 {title}\n\n"
            f"💿 {output_mb} MB - تم التحويل\n"
            f"✅ PlayZone"
        )
...
async def handle_text(message: Message):
    ...
    if not is_url(text):
        await message.answer(
            f"🔎 نتيجة البحث عن:\n{text}\n\n"
            "⚠️ لا أستطيع البحث والتحميل تلقائياً من المنصات.\n\n"
            "✅ أرسل الملف نفسه هنا، أو أرسل رابط مباشر لملف صوتي/فيديو.",
            reply_markup=download_home_menu()
        )
        return
    ...
    if not allowed:
        if why == "blocked":
            await message.answer(
                "⚠️ هذا الرابط من منصة غير مسموح التحميل منها مباشرة.\n\n"
                "✅ الحل الأسهل:\n"
                "أرسل ملف الفيديو أو الصوت هنا، وسأحوله لك إلى MP3 فوراً.",
                reply_markup=download_home_menu()
            )
        else:
            await message.answer(
                "⚠️ لا يمكن تحميل هذا الرابط مباشرة.\n\n"
                "الرابط يجب أن يكون من موقع مسموح أو رابط ملف مباشر ينتهي مثلاً بـ:\n"
                ".mp3 / .mp4 / .m4a / .wav\n\n"
                "✅ أو أرسل الملف نفسه للبوت.",
                reply_markup=download_home_menu()
            )
        return
    ...
    try:
        ...
        await notify_admin(
            f"✅ رابط تم تحويله بنجاح\n\n"
            f"👤 المستخدم: {user_id}\n"
            f"🌐 المصدر: {get_domain(text)}\n"
            f"💾 الحجم: {output_mb}MB"
        )
