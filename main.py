...
def get_ydl_options(job_dir: Path | None = None, progress_data: dict | None = None, mode: str = "video"):
    ...
    if mode == "audio":
        # نفس فكرتك: نطلب أفضل صوت خام متاح ثم نحوله محلياً إلى MP3
        opts["format"] = "bestaudio/best"
    else:
        # نفس الهبوط التدريجي للفيديو مع حد 720p حتى يبقى مناسباً لحد 50MB
        opts["format"] = (
...
async def start_download_from_callback(query, context: ContextTypes.DEFAULT_TYPE, request: dict, mode: str):
    ...
    try:
        ...
        # أخذ غلاف المعاينة محلياً للصوت مثل كودك، حتى يظهر كـ thumbnail في Telegram
        thumb_url = request.get("thumb_url")
        local_thumb = None
        if thumb_url and is_valid_url(thumb_url):
            try:
