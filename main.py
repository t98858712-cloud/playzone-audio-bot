...
MAX_GLOBAL_DOWNLOADS = int(os.getenv("MAX_GLOBAL_DOWNLOADS", "3"))
MAX_WORKERS = int(os.getenv("MAX_WORKERS", str(min(12, max(4, CPU_COUNT * 3)))))
YTDLP_FRAGMENTS = int(os.getenv("YTDLP_FRAGMENTS", "8"))
...
def convert_to_mp3_local(input_file: Path, output_file: Path) -> bool:
    try:
        cmd = [
            ...
            "-threads", "0", "-i", str(input_file),
            "-vn", "-ar", "44100", "-ac", "2", "-b:a", "320k",
            "-map", "a:0?", "-async", "1", str(output_file),
        ]
        subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True, timeout=300)
    ...
    try:
        ...
        if duration_raw and duration_raw > MAX_DURATION_SECONDS:
            await status.edit_text(
                f"❌ المقطع طويل جداً.\n\nالحد الأعلى: {format_duration(MAX_DURATION_SECONDS)}"
            )
            return
        ...
        await status.edit_text("❌ انتهى وقت فحص الرابط.\n\nحاول مرة أخرى أو أرسل رابطاً أقصر.")
    except Exception as e:
...
async def start_download_from_callback(query, context: ContextTypes.DEFAULT_TYPE, request: dict, mode: str):
    ...
    try:
        ...
        await asyncio.wait_for(
            loop.run_in_executor(IO_EXECUTOR, lambda: execute_download(url, mode, job_dir, progress_data)),
...
def main():
    ...
    logger.info(f"MAX_GLOBAL_DOWNLOADS={MAX_GLOBAL_DOWNLOADS}, MAX_WORKERS={MAX_WORKERS}, YTDLP_FRAGMENTS={YTDLP_FRAGMENTS}")
