...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "أرسل رابط فيديو أو صوت من أي منصة تريدها (يوتيوب، تيك توك، إنستغرام، فيسبوك...) للبدء!"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق الأولي من أن الرسالة تحتوي على رابط فعلي
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_message = await query.edit_message_text("⏳ جاري تحليل الرابط وسحب الملف...")
    out_tmpl = str(job_dir / "%(title).80s.%(ext)s")

    # إعدادات عامة ومرنة لجميع المواقع
    base_ydl_opts = {
        "outtmpl": out_tmpl,
        "quiet": True,
        "noplaylist": True,
    ...
    # 💡 تخصيص يوتيوب: نطبق الكوكيز ومحاكاة الأندرويد فقط إذا كان الرابط يخص يوتيوب
    ...
    # إعدادات الجودة والصيغة حسب الاختيار
    if choice == "mp3":
        ydl_opts = {
            **base_ydl_opts, 
        ...
        # تحميل فيديو مباشر مدمج الصوت ومناسب للتليجرام
        ydl_opts = {
            **base_ydl_opts, 
    ...
    try:
        await status_message.edit_text("📥 جاري التحميل من المنصة...")
        info = await loop.run_in_executor(None, lambda: yt_dlp.YoutubeDL(ydl_opts).extract_info(url, download=True))
        
        title = info.get("title", "ملف ميديا")
        file_path = find_downloaded_file(job_dir)

        if not file_path or not file_path.exists():
            await status_message.edit_text("❌ لم يتم العثور على الملف بعد تحميله.")
            return

        if file_path.stat().st_size > MAX_TELEGRAM_SIZE:
            await status_message.edit_text("❌ حجم الملف أكبر من 50MB (الحد الأقصى لرفع الملفات عبر البوتات العادية).")
            return
