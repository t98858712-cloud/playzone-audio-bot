...
logger = logging.getLogger("PlayZoneDownloadBot")
...
async def send_preview(update: Update, thumb: str, caption: str, keyboard: InlineKeyboardMarkup):
    if thumb:
        try:
            ...
            logger.warning(f"فشل إرسال صورة المعاينة، سيتم إرسال نص فقط: {e}")
...
async def handle_incoming_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status.edit_text(
            "❌ لم أتمكن من قراءة هذا الرابط.\n"
            "تأكد أن المقطع عام وغير محمي، ثم جرّب مرة ثانية."
        )
    ...
    try:
        ...
        if file_size > MAX_TELEGRAM_SIZE:
            ...
            await edit_message_smart(
                query.message,
                "❌ حجم الملف أكبر من الحد المسموح للبوتات في تيليجرام.",
                reply_markup=None,
            )
            return
