...
# --- جلب التوكن من بيئة Railway، وتثبيت يوزر البوت يدوياً ---
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN", "")
BOT_USERNAME = "MusicPlayZoneBot" # تم تثبيت اليوزر هنا يدوياً كما طلبت
# -------------------------------------------------------------
...
app = FastAPI(title="PlayZone Cloud", description="منصة التحميل والميديا السحابية المربوطة بتيليجرام")
init_db()
...
# ==========================================
# الواجهة الاحترافية (مع نظام تيليجرام الذكي)
# ==========================================
INDEX_HTML = f"""
<!DOCTYPE html>
...
<head>
    ...
    <!-- مكتبة تيليجرام لتشغيل السحب التلقائي للآي دي (Zero-Intervention) -->
...
<body class="antialiased">
    ...
    <!-- نافذة ربط تيليجرام الذكية (تظهر فقط لمن يتصفح خارج تيليجرام) -->
    <div id="tgModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] hidden flex-col items-center justify-center p-4">
        <div class="bg-slate-800 border border-slate-700 p-6 rounded-3xl max-w-sm w-full text-center shadow-2xl">
            ...
            <button onclick="window.open('https://t.me/{BOT_USERNAME}', '_blank')" class="btn bg-blue-600 hover:bg-blue-500 text-white w-full mb-4 shadow-lg">
                <i class="fas fa-robot"></i> 1. افتح البوت لنسخ الـ ID
            </button>
    ...
    <div class="max-w-6xl mx-auto p-4 space-y-6">
        <header class="glass p-4 rounded-2xl flex justify-between items-center shadow-lg">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-primary/20 rounded-xl flex items-center justify-center text-primary text-xl">
                    <i class="fas fa-bolt"></i>
                </div>
                <h1 class="text-2xl font-black tracking-wide text-white">Play<span class="text-primary">Zone</span></h1>
            </div>
            <div class="flex gap-2">
                <a href="https://t.me/{BOT_USERNAME}" target="_blank" class="w-10 h-10 rounded-xl bg-blue-500/20 hover:bg-blue-500/40 text-blue-400 flex items-center justify-center transition-colors tooltip" title="التواصل مع البوت">
                    <i class="fab fa-telegram-plane"></i>
                </a>
            </div>
        </header>
            ...
            <div class="lg:col-span-1 space-y-6">
                <section class="glass rounded-3xl p-6 shadow-xl relative overflow-hidden">
                    ...
                    <div class="space-y-4 relative z-10">
                        ...
                        <button onclick="processInput()" id="mainBtn" class="btn bg-blue-600 hover:bg-blue-500 text-white w-full shadow-lg shadow-blue-600/30">
                            <i class="fas fa-search"></i> إيجاد المقطع
                        </button>
                    </div>
    ...
    <script>
        // --- ميزة التدخل الصفري (Zero-Intervention) لجلب الـ ID تلقائياً ---
        ...
        function renderPage() {{
            ...
            if(filteredLibrary.length === 0) {{
                container.innerHTML = '<div class="col-span-full text-center py-16 text-slate-500"><i class="fas fa-box-open text-5xl mb-4 opacity-30 block"></i>لا توجد ملفات.</div>';
                return;
            }}
            ...
            if (totalPages > 1) {{
                ...
                paginator.innerHTML += `<span class="text-sm font-mono px-3">صفحة ${{currentPage}} من ${{totalPages}}</span>`;
                paginator.innerHTML += `<button onclick="currentPage++; renderPage()" ${{currentPage === totalPages ? 'disabled' : ''}} class="w-8 h-8 rounded bg-slate-800 hover:bg-primary disabled:opacity-30"><i class="fas fa-chevron-left"></i></button>`;
            }}
        }}

        function toggleFavorite(id) {{
            const index = myLibrary.findIndex(f => f.id === id);
            if(index > -1) {{
        ...
        // --- دوال النافذة الذكية لتيليجرام ---
        function openTgModal(url, isAudio) {{
            pendingTgFileUrl = url;
            pendingTgIsAudio = isAudio;
        ...
        function saveTgId() {{
            ...
            localStorage.setItem('pz_tg_chat_id', id);
            closeTgModal();
            sendToTelegram(pendingTgFileUrl, pendingTgIsAudio); 
        }}

        async function sendToTelegram(fileUrl, isAudio) {{
            let chatId = localStorage.getItem('pz_tg_chat_id');
            if (!chatId) {{
                openTgModal(fileUrl, isAudio);
                return;
            }}
            ...
            try {{
                ...
                if (data.success) {{
                    ...
                    showToast("❌ فشل الإرسال: " + (data.error || "تأكد من بدء محادثة مع البوت"), "error");
        ...
        function playGlobalAudio(fileId) {{
            ...
            audioEl.play().catch(e => showToast("تعذر التشغيل", "error"));
                    ...
                    if(data.success && data.entries.length) {{
                        ...
                        data.entries.forEach(v => {{
                            if(v.id) box.innerHTML += `<div onclick="renderPreview('https://youtube.com/watch?v=${{v.id}}')" class="p-2 bg-slate-800/50 hover:bg-slate-700 rounded-xl cursor-pointer flex gap-3 items-center border border-transparent hover:border-primary/30">
                                <img src="${{v.thumbnails ? v.thumbnails[0].url : ''}}" class="w-16 h-10 rounded object-cover shadow aspect-video">
        ...
        async function renderPreview(url) {{
            ...
            try {{
                ...
                }} else showToast("تعذر قراءة المقطع", "error");
                    ...
                    const interval = setInterval(async ()=>{{
                        try {{
                            ...
                            if(prog.status === 'downloading') {{
                                ...
                                clearInterval(interval); document.getElementById('progStatus').innerHTML = '<span class="text-primary">اكتمل التحميل!</span>';
                                ...
                                localStorage.setItem('pz_enterprise_library', JSON.stringify(myLibrary));
                                applyFilters(); showToast("تم الحفظ بالمكتبة", "success");
                            }} 
                            else if(prog.status === 'error') {{ clearInterval(interval); document.getElementById('progStatus').innerHTML = '<span class="text-red-500">فشل!</span>'; }}
                        }} catch(err) {{}}
                    }}, 1500);
                }}
            }} catch(e) {{ showToast("تأكد من اتصالك", "error"); }}
        }}
    </script>
</body>
</html>
...
def bg_download(job_id: str, url: str, mode: str, res: str):
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl: 
            ...
            PROGRESS_CACHE[job_id] = {"status": "completed", "url": f"/files/{filename}", "title": info.get('title', 'مقطع PlayZone'), "thumb": info.get('thumbnail', ''), "is_audio": mode == 'audio'}
    except Exception as e: PROGRESS_CACHE[job_id] = {"status": "error", "error": str(e)}
...
async def send_to_telegram(req: TelegramRequest):
    try:
        ...
        if not TELEGRAM_TOKEN:
            return {"success": False, "error": "التوكن غير متوفر في السيرفر! تأكد من إعداد TELEGRAM_TOKEN في Railway."}
