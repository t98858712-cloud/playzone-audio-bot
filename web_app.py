...
<head>
    ...
    <style>
        /* Custom styles for light/dark variables */
...
<body class="font-sans p-4">
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <!-- قسم العنوان وزر الوضع الليلي/النهاري -->
        ...
        <div class="theme-card p-6 rounded-2xl shadow-lg text-center">
            ...
            <div class="relative mb-4">
                <input type="text" id="url" placeholder="الرابط أو كلمة البحث..." class="w-full theme-input p-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary transition-all">
            </div>
            <button onclick="processInput()" id="mainBtn" class="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition-all shadow-md w-full md:w-auto">
            ...
            <div id="previewBox" class="hidden mt-6 p-4 theme-card rounded-xl text-right flex flex-col md:flex-row gap-4 items-center border">
                ...
                <div class="flex-1 w-full">
                    ...
                    <div id="adGate" class="bg-green-100 dark:bg-green-900/30 border border-primary p-4 rounded-xl text-center mb-4">
                        ...
                        <a href="{{ ad_link }}" target="_blank" onclick="startAdTimer()" class="bg-primary text-gray-900 px-5 py-2 rounded-lg font-bold text-sm inline-block shadow-sm hover:scale-105 transition-transform"><i class="fas fa-eye ml-1"></i> تصفح الإعلان</a>
                        <button id="verifyBtn" onclick="unlockDownload()" disabled class="bg-gray-300 dark:bg-gray-700 text-gray-500 px-5 py-2 rounded-lg font-bold text-sm inline-block mr-2 cursor-not-allowed transition-colors"><i class="fas fa-lock ml-1"></i> مقفول</button>
                    </div>

                    <div id="dlOptions" class="hidden space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <select id="mode" onchange="toggleRes()" class="theme-input p-3 rounded-xl text-sm font-medium w-full">
                                <option value="video">🎬 مقطع مرئي (فيديو)</option>
                                <option value="audio">🎵 مقطع صوتي فقط</option>
                            </select>
                            <select id="resolution" class="theme-input p-3 rounded-xl text-sm font-medium w-full">
                                <option value="360">جودة عادية</option>
                                <option value="720" selected>جودة عالية (HD)</option>
                                <option value="best">أفضل جودة متاحة</option>
                            </select>
                        </div>
                        <button onclick="downloadMedia()" class="w-full bg-primary text-gray-900 font-bold py-3 rounded-xl hover:bg-green-500 transition-colors shadow-md text-lg"><i class="fas fa-cloud-download-alt ml-2"></i> حمل الآن</button>
                    </div>
                </div>
            </div>
        ...
        <div class="theme-card p-6 rounded-2xl shadow-lg">
            <div class="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
                ...
                <div class="flex gap-2">
                    <button onclick="location.reload()" class="px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors font-bold text-sm" title="تحديث السجل"><i class="fas fa-sync ml-1"></i> تحديث</button>
                    <a href="/admin" class="px-4 py-2 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded-lg font-bold text-sm transition-colors hover:bg-blue-200 dark:hover:bg-blue-800"><i class="fas fa-cog ml-1"></i> الإدارة</a>
                </div>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                {% for file in files %}
                <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-primary dark:hover:border-primary transition-all shadow-sm">
                    ...
                    <div class="flex gap-2 mt-4">
                        <a href="{{ file.url }}" download class="flex-1 text-center bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 py-2 rounded-lg text-sm font-bold transition-colors"><i class="fas fa-download ml-1"></i> تحميل للجهاز</a>
                        <button onclick="shareLink('{{ file.url }}')" class="flex-none px-4 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg transition-colors" title="نسخ رابط المقطع"><i class="fas fa-link"></i></button>
                    </div>
                </div>
                {% endfor %}
                {% if not files %}
    ...
    <script>
        ...
        function updateThemeIcon(isDark) {
            ...
            if(isDark) { icon.className = 'fas fa-sun text-xl text-yellow-400'; } 
            else { icon.className = 'fas fa-moon text-xl text-gray-600'; }
        }
        if (localStorage.getItem('theme') === 'light') { document.documentElement.classList.remove('dark'); updateThemeIcon(false); }
        else { updateThemeIcon(true); }

        // --- الإشعارات المنبثقة ومشاركة الروابط ---
        ...
        function shareLink(url) {
            ...
            navigator.clipboard.writeText(fullUrl).then(() => showToast("🔗 تم نسخ رابط المقطع بنجاح!"));
        }
        ...
        async function processInput() {
            ...
            btn.innerHTML = '<i class="fas fa-spinner fa-spin ml-2"></i> لحظات...';
            if (input.startsWith('http')) await fetchPreview(input);
                ...
                if(data.success) {
                    ...
                    data.entries.forEach(v => box.innerHTML += `<div onclick="fetchPreview('https://youtube.com/watch?v=${v.id}')" class="bg-gray-50 dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer mt-2 hover:border-primary transition-colors flex items-center justify-between"><p class="font-bold text-sm truncate flex-1 ml-2 text-gray-800 dark:text-gray-200">${v.title}</p><span class="text-xs bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded text-gray-600 dark:text-gray-400">⏱ ${v.duration} ث</span></div>`);
        ...
        async function fetchPreview(url) {
            ...
            if(data.success) {
                ...
                vBtn.disabled = true; vBtn.className = "bg-gray-300 dark:bg-gray-700 text-gray-500 px-5 py-2 rounded-lg font-bold text-sm inline-block mr-2 cursor-not-allowed"; vBtn.innerHTML = '<i class="fas fa-lock ml-1"></i> مقفول'; adWatched = false;
            } else showToast("❌ تعذر قراءة الرابط المدخل");
        }
        function startAdTimer() { 
            showToast("⏳ يرجى تصفح الإعلان 12 ثانية لتفعيل الزر");
            setTimeout(()=>{
                adWatched=true; 
                const vBtn = document.getElementById('verifyBtn'); 
                vBtn.disabled=false; 
                vBtn.className="bg-blue-600 text-white px-5 py-2 rounded-lg font-bold text-sm inline-block mr-2 shadow-md hover:bg-blue-700 transition-colors"; 
                vBtn.innerHTML="<i class='fas fa-unlock ml-1'></i> افتح القفل";
                showToast("🔓 اكتمل الوقت، يمكنك الآن فتح القفل!");
            }, 12000); 
        }
