...
def get_ydl_options(job_dir: Path | None = None, progress_data: dict | None = None, mode: str = "video", resolution: str = "720"):
    opts = {
        ...
        "http_headers": { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)", "Accept-Language": "en-US,en;q=0.9" }
    }
    if mode == "audio": opts["format"] = "bestaudio/best"
    else:
        max_fs = "50M" if not LOCAL_API_URL else "2000M"
        opts["format"] = f"bestvideo[vcodec^=avc1][height<={resolution}][filesize<?{max_fs}]+bestaudio/best"
...
def extract_metadata(url: str):
    ...
    opts["skip_download"] = True; opts["extract_flat"] = False; opts.pop("format", None) 
    with yt_dlp.YoutubeDL(opts) as ydl: return ydl.extract_info(url, download=False)

def search_youtube(query: str, limit: int = 30):
    opts = { "quiet": True, "extract_flat": True, "no_warnings": True, "ignoreerrors": True, "extractor_args": { "youtube": { "player_client": ["android", "ios", "tv"], "player_skip": ["web", "mweb"] } } }
    if cookie_file_is_usable(COOKIES_FILE): opts["cookiefile"] = str(COOKIES_FILE)
    combined_entries = []; seen_ids = set()
    try:
        ...
        for entry in entries:
            if entry and entry.get('id') and entry['id'] not in seen_ids:
                combined_entries.append(entry); seen_ids.add(entry['id'])
    except Exception as e: logger.warning(f"Engine ytsearch failed: {e}")
...
def download_hook(progress_data: dict):
    def hook(d):
        ...
        if d.get("status") == "downloading":
            ...
            else: progress_data["text"] = _t("msg_dl_progress_no_total", lang, downloaded=format_size(downloaded, lang))
        elif d.get("status") == "finished": progress_data["text"] = _t("msg_dl_finished", lang)
    return hook
...
def download_thumbnail_safely(thumb_url: str, output_path: Path) -> Path | None:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=6) as response: data = response.read(2 * 1024 * 1024 + 1)
...
async def youtube_health_monitor(app: Application):
    while True:
        ...
        try:
            if not cookie_file_is_usable(COOKIES_FILE): continue
            opts = { "quiet": True, "extract_flat": True, "cookiefile": str(COOKIES_FILE) }
            with yt_dlp.YoutubeDL(opts) as ydl: ydl.extract_info("https://www.youtube.com/watch?v=BaW_jenozKc", download=False)
