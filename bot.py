...
import core.state as sec
from database.connection import init_db
from database.blacklist import load_banned_users
from utils.cleaner import _cleanup_old_downloads_sync
from utils.helpers import parse_admin_ids

from handlers.commands import start, toggle_lang_command, show_playzone_links
from handlers.messages import handle_incoming_text
from admin.panel import admin_panel
from handlers.clicks import handle_callbacks
from video.downloader import youtube_health_monitor, start_health_check_server
...
async def post_init(app: Application):
    user_commands = [BotCommand("start", "بدء / Start"), BotCommand("language", "تغيير اللغة / Toggle Language"), BotCommand("links", "الروابط / Links")]
    admin_commands = user_commands + [BotCommand("admin", "لوحة التحكم / Admin Panel")]
    try:
        await app.bot.set_my_commands(user_commands)
        for admin_id in parse_admin_ids():
            try: await app.bot.set_my_commands(admin_commands, scope=BotCommandScopeChat(admin_id))
            except: pass
...
def main():
    ...
    init_db(); sec.BANNED_USERS_CACHE = load_banned_users(); _cleanup_old_downloads_sync()
