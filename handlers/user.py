...
from core.exceptions import ContentRestrictedException, MediaDownloadException
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    first_name = esc(update.effective_user.first_name or "")
    await update.message.reply_text(_t("msg_start", lang, first_name=first_name), reply_markup=user_main_keyboard(lang), parse_mode="HTML", disable_web_page_preview=True)
...
async def handle_incoming_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(uid):
        ...
        if len(reqs) > 12:
            ...
            await alert_admins_live(context.bot, f"🚨 <b>نظام الحماية:</b> تم حظر المستخدم <code>{uid}</code> مؤقتاً بسبب السبام العالي.")
    ...
    except ContentRestrictedException as cre:
        logger.warning(f"محتوى مقيد: {cre}")
        await status.edit_text(f"🔒 <b>عذراً، هذا المقطع غير متاح:</b>\n{cre.message}", parse_mode="HTML")
    except MediaDownloadException as mde:
        logger.warning(f"خطأ تنزيل مؤسسي: {mde}")
        await status.edit_text(_t("msg_link_error", lang))
    except Exception as e:
        logger.warning(f"فشل عام في معالجة الرابط: {e}")
        await status.edit_text(_t("msg_link_error", lang))
