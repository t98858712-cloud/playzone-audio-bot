...
from utils.helpers import is_admin, _force_cleanup_all_sync, format_size, esc, get_user_lang
...
async def process_user_info(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: int):
    ...
    try:
        ...
        if u_doc.exists:
            u = u_doc.to_dict()
            status = _t("txt_banned", lang) if uid in BANNED_USERS_CACHE else _t("txt_active", lang)
            name = esc(f"{u.get('first_name', '')} {u.get('last_name', '')}".strip())
            text = _t("msg_user_info", lang, id=u.get('id', uid), name=name, username=u.get('username', 'N/A'), status=status)
            await update.message.reply_text(text, parse_mode="HTML")
        else:
            await update.message.reply_text(_t("msg_user_not_found", lang))
