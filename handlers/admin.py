...
from utils.keyboards import admin_main_keyboard, admin_broadcast_menu, admin_system_menu, admin_users_menu, admin_security_menu, admin_cancel_action_keyboard
from locales.language import _t
from core.config import BASE_DOWNLOAD_DIR, COOKIES_FILE, EXECUTOR
...
async def handle_admin_callbacks(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_main_back":
        ...
        try:
            ...
            await edit_message_smart(query.message, "✅ <b>تم تحديث مكتبة yt-dlp لآخر إصدار بنجاح.</b>", reply_markup=admin_system_menu(lang))
        except Exception as e:
            await edit_message_smart(query.message, f"❌ <b>فشل التحديث:</b>\n<code>{e}</code>", reply_markup=admin_system_menu(lang))
    elif data == "adm_req_cookie":
        context.user_data["awaiting_cookie"] = True
        await query.answer()
        return await edit_message_smart(query.message, "🍪 <b>تحديث الكوكيز:</b>\n\nالرجاء إرسال ملف <code>cookies.txt</code> الآن كملف (Document).", reply_markup=admin_cancel_action_keyboard(lang))
        ...
        await query.answer()
        return await edit_message_smart(query.message, "🔍 <b>الاستعلام عن مستخدم:</b>\n\nالرجاء إرسال أيدي (ID) المستخدم الآن.", reply_markup=admin_cancel_action_keyboard(lang))
        ...
        optimize_db()
        return await edit_message_smart(query.message, "✅ <b>تم ضغط وتحسين قاعدة البيانات بنجاح!</b>", reply_markup=admin_security_menu(lang))
    elif data == "adm_clean":
...
async def handle_admin_inputs(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if context.user_data.get("awaiting_cookie"):
        if update.message.document:
            ...
            await update.message.reply_text("✅ <b>تم استقبال وتحديث ملف الكوكيز بنجاح.</b>", reply_markup=admin_main_keyboard(lang), parse_mode="HTML")
        else:
    ...
    if context.user_data.get("awaiting_uid"):
        if update.message.text:
            ...
            try:
                ...
                if u_doc.exists:
                    u = u_doc.to_dict()
                    status = "🔴 محظور" if int(target_uid) in BANNED_USERS_CACHE else "🟢 نشط"
