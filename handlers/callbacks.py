...
from utils.helpers import is_admin, clean_title, format_duration, format_size, esc, ensure_pending_requests, trim_old_pending_requests
...
from services.downloader import download_thumbnail_safely, execute_download, run_progress_updates
...
from handlers.user import render_search_page, extract_metadata
import logging
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("selsrc:"):
        ...
        try:
            ...
            from utils.helpers import get_artist, format_size, get_thumbnail, get_largest_estimated_size
            ...
            ensure_pending_requests(context)[request_id] = {"url": url, "title": title, "artist": artist, "duration": duration_raw, "thumb_url": thumb, "created_at": int(time.time())}
            ...
            if thumb and (thumb.startswith("http://") or thumb.startswith("https://")):
                try: await context.bot.send_photo(chat_id=uid, photo=thumb, caption=caption, reply_markup=keyboard, parse_mode="HTML")
                except Exception: await context.bot.send_message(chat_id=uid, text=caption, reply_markup=keyboard, parse_mode="HTML", disable_web_page_preview=True)
    ...
    try:
        ...
        async with DOWNLOAD_SEMAPHORE:
            ...
            with open(target_file, "rb") as f:
                ...
                else: await context.bot.send_video(chat_id=query.message.chat_id, video=f, caption=caption, supports_streaming=True, duration=duration, reply_markup=media_keyboard, parse_mode="HTML", read_timeout=120, write_timeout=120)
        ...
        stat_inc_sync("failed")
        from utils.helpers import alert_admins_live
